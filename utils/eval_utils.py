...
def prepare_environment(args, cfg):
    ...
    if args.foa:
        model_cfg['args']['audio_backbone_args']['inp_channels']=7
    if args.beam:
        #args.model_cfg['dataset']['audio_input'] = "beamforming"
        model_cfg['args']['audio_backbone_args']['inp_channels']=1

    model_cfg['model_dir'] =args.type + '/' + model_cfg['model_dir']
    #print(model_cfg['model_dir'])
        #print( model_cfg['args']['checkpoint'])


    if args.checkpoint_dir is None:
        if args.debug:
            eval_dir = '{}/{}/{}/eval-{}/fold-{:02d}'.format('debug_eval',model_cfg['model_dir'], model_cfg['name'], cfg['benchmark'], cfg['dataset']['fold'])
        else:
            eval_dir = '{}/{}/{}/eval-{}/fold-{:02d}'.format('eval',model_cfg['model_dir'], model_cfg['name'], cfg['benchmark'], cfg['dataset']['fold'])
    else:
...
def build_model(feat_cfg, cfg, eval_dir, args, logger=None, return_ckp=False, scratch=False):
    ...
    if str(4) in args.type:
        #feat_cfg['args']
        #checkpoint_fn= feat_cfg['model_dir']+'/'+feat_cfg['name']+'/checkpoint.pth.tar'
        #feat_cfg['args']['checkpoint']= 'train/C1Y0B0F0/'+feat_cfg['args']['checkpoint']
        feat_cfg['args']['checkpoint'] = None
    ...
    if scratch:
        ...
        #checkpoint_fn = '{}/{}/model_best.pth.tar'.format(feat_cfg['model_dir'], feat_cfg['name'])
